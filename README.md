...
- `src/infrastructure/expo/content/training.yaml`: guion de la conversación. Cada entrada es `mestre`, `student` o `single-choice`; los textos `|-` conservan los saltos de línea de los versos. Editarlo actualiza el contenido mediante Metro.
...
El YAML se transforma durante el empaquetado y se valida con Zod al cargar y en los tests. Para añadir conversación, agrega entradas `mestre` o `student`; para añadir retos, usa una entrada `single-choice` siguiendo el esquema de `training.yaml`. Las etiquetas y frases del cierre se definen en `completionLabel`, `completion`, `completionTitle`, `completionSummary` y `restartAction`; el resumen incluye exactamente un marcador `{COUNT}`. Cambiar el ID de la lección invalida el progreso anterior. No hay cuentas ni sincronización entre dispositivos.
