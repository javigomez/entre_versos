...
import { answer, initialProgress } from '../domain/session';
...
export type PendingReply = { nextProgress: Progress; nextMessages: Message[]; messageId: string; controlId: string };
export type FlowState = { progress: Progress; messages: Message[]; revealed: number; phase: Phase; token: number; anchorId: string | null; pending: PendingReply | null };
...
function phaseFor(session: Session, state: Pick<FlowState, 'progress' | 'messages' | 'revealed'>): Phase {
  ...
  if (state.progress.completed.length === session.script.filter(item => item.type === 'single-choice').length) return 'finished';
  return 'waiting-choice';
}

export function createFlow(session: Session, progress: Progress, restored: boolean): FlowState {
  const messages = messagesFor(session, progress);
  const revealed = restored ? messages.length : 0;
  const state = { progress, messages, revealed };
  const first = messages[0]?.id ?? null;
  return { ...state, phase: phaseFor(session, state), token: 0, anchorId: first, pending: null };
}

export function reduceFlow(session: Session, state: FlowState, event: FlowEvent): FlowState {
  if (event.type === 'RESET') {
    const progress = initialProgress(session);
    const messages = messagesFor(session, progress);
  ...
  switch (event.type) {
    case 'START': {
      ...
      return { ...next, phase: phaseFor(session, next) };
    }
    ...
    case 'ANSWER': {
      ...
      const nextProgress = answer(session, state.progress, event.optionId);
      if (nextProgress === state.progress) return state;
      const nextMessages = messagesFor(session, nextProgress);
